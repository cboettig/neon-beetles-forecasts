...
```{r}
past <-  targets_ts %>% filter(time < forecast_date)
```
...
null_abundance %>% filter(siteID %in% first4)  %>% autoplot(past) + ggtitle("abundance")
```
...
forecast_raw <- inner_join(efi_format(null_richness), 
...
```{r}
forecast <- forecast_raw %>% mutate(time = iso_week(time) )
```
